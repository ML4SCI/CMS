...
from channel_treatments import (
    maybe_apply_channel_treatment as _apply_ct,
    channel_treatment_of as _ct_of,
)
...
class DepthViTModule(L.LightningModule):
    def __init__(
        ...
        self.model = DepthViT(
            ...
            pos_embed_2d=bool(model_cfg.get("pos_embed_2d", False)),
                                                                                  
                                                                                
            compile_blocks=(bool(model_cfg.get("compile_blocks", False))
                            if _ct_of(model_cfg) == "asym" else False),
        )
        _apply_ct(self.model, model_cfg)
...
class IJEPA_DepthViTModule(L.LightningModule):
    def __init__(
        ...
        def _build_depthvit(mcfg):
            return DepthViT(
                ...
                pos_embed_2d=bool(mcfg.get("pos_embed_2d", False)),
    ...
    def _sample_mask_energy_weighted(
        ...
        patches = patches.permute(0, 2, 4, 1, 3, 5).reshape(B, L, -1)                   
        patch_energy = patches.norm(dim=-1)          
...
class UnifiedDataModule(L.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if str(cfg_local["data"].get("format", "")).lower() == "cifar100":
            from data_cifar100 import make_loaders as _cifar100_make_loaders
            self._train_loader, self._val_loader = _cifar100_make_loaders(cfg_local)
        else:
