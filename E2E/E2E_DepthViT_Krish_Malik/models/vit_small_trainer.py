"""
ViT-Small trainer for HLS4ML LHC jets (tier-4, 22M-scale baseline).
...
Only the LightningModule is new (ViTSmallModule). Single-phase
classification only — no iJEPA support (ViT-Small is the matched
baseline for the 22M tier, not a subject of pretraining ablations).

Structurally identical to vit_tiny_trainer.py; only the imported model
class (ViTSmall) and default model dims differ.

Usage:
    python3 -m torch.distributed.run --standalone --nproc_per_node=4 \
        vit_small_trainer.py --config configs/jets_150p_vit_small_90epoch.json

Smoke test (caps to N batches per phase):
    python3 -m torch.distributed.run --standalone --nproc_per_node=4 \
        vit_small_trainer.py --config configs/jets_150p_vit_small_smoke.json \
        --smoke-test 5
"""
...
class ViTSmallModule(L.LightningModule):
    """Classification-only Lightning module for ViT-Small."""
...
def main():
    ...
    # ViT-Small is single-phase; if a phases array is given, take the first.
    phases_cfg = cfg.get(
        "phases",
        [{
            "name": "cls_finetune",
    ...
    print(f"[ViTSmall] trainable params: {n_p:,}  ({n_p/1e6:.2f}M)")
    ...
    if ckpt_path:
        print(f"[ViTSmall] resuming from {ckpt_path}")
