...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size: int = 100, patch_size: int = 10,
                 in_channels: int = 2, embed_dim: int = 192):
        super().__init__()
...
class ViTTiny(nn.Module):
    ...
    Defaults reproduce the standard ViT-Tiny configuration
    (12 layers, 192 hidden, 3 heads, MLP ratio 4) but with patch
...
if __name__ == "__main__":
    m = ViTTiny()
    n_p = m.param_count()
    print(f"ViT-Tiny params: {n_p:,}  ({n_p/1e6:.2f}M)")
