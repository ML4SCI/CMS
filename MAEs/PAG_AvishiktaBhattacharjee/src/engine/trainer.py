...
class Trainer:
    ...
    plots_dir: str, optional
        Folder for the PNG figures; each model gets its own subfolder (e.g. plots/LorentzParT). Overrides config if provided.
    ...
    def __init__(
        ...
        if config is not None:
            ...
            self.scheduler = get_scheduler_from_config(scheduler_config, SCHEDULER_REGISTRY, self.optimizer) if scheduler_config else None
            callbacks_config = callbacks if callbacks is not None else config.callbacks
            self.callbacks = get_callbacks_from_config(callbacks_config, CALLBACK_REGISTRY) if callbacks_config else []
            ...
            self.plots_dir = plots_dir if plots_dir is not None else config.plots_dir
            ...
            self.callbacks = get_callbacks_from_config(callbacks, CALLBACK_REGISTRY) if callbacks is not None else []
            ...
            self.plots_dir = plots_dir if plots_dir is not None else 'plots'
        ...
        # PNG figures go to a separate folder per model, e.g. plots/LorentzParT
        self.outputs_dir = os.path.join(self.plots_dir, self.model_name)
        if self.save_fig:
