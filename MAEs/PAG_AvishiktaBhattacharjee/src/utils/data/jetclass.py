...
from .normalize import assert_uniform_scaling, shared_scale
...
class JetClassDataset(Dataset):
    ...
    def __init__(
        ...
        if norm_dict is not None:
            assert_uniform_scaling(normalize)
    ...
    def __getitem__(self, idx: int) -> Tuple[Tensor, ...]:
        ...
        if self.mask_mode is not None:
            ...
            if self.norm_dict is not None:
                scale = shared_scale(self.norm_dict)
                for i, feature in enumerate(feature_names):
                    if self.normalize[i]:
                        mean, std = self.norm_dict[feature]

                        if i == 0 or i == 3:  # pT and energy: ONE shared scale, see shared_scale()
                            masked_particles[:, i] = masked_particles[:, i] / scale
                            masked_targets[:, i] = masked_targets[:, i] / scale
                        else:
            ...
            # Copy first: `particles` is a view, so normalizing in place would rescale the
            # stored jets again on every access (i.e. every epoch)
            particles = particles.copy()
            ...
            if self.norm_dict is not None:
                ...
                for i, feature in enumerate(feature_names):
                    if self.normalize[i]:
                        mean, std = self.norm_dict[feature]
                        if i == 0 or i == 3:  # pT and energy: ONE shared scale, see shared_scale()
                            # Was a no-op, so classification fed the encoder raw GeV while
                            # pretraining fed it pT/mean -- and denormalize() then scaled a
                            # second time, inflating the jet mass reaching the gate.
                            particles[:, i] = particles[:, i] / scale
                        else:
...
class LazyJetClassDataset(JetClassDataset):
    ...
    def __init__(
        ...
        if self.norm_dict is not None:
            ...
            for i, k in enumerate(self._feat_names):
                ...
                # pT / energy: ONE shared scale (keeps the 4-vector uniform, see
                # shared_scale()); eta/phi: (x-mean)/std
                means.append(scale if i in (0, 3) else m)
    ...
    def _apply_norm_inplace(self, arr: np.ndarray) -> None:
        ...
        # pT/energy: x /= shared scale ; eta/phi: (x-mean)/std
        # self._means[0] and self._means[3] both hold the shared scale (see __init__)
